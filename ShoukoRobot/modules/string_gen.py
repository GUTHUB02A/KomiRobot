...
@pbot.on_message(filters.private & filters.command("generate"))
async def genStr(_, msg: Message):
    ...
    try:
        ...
        return
    api_id = api.text
    hash = await pbot.ask(chat.id, HASH_TEXT)
    ...
    api_hash = hash.text
    while True:
        number = await pbot.ask(chat.id, PHONE_NUMBER_TEXT)
        if not number.text:
            continue
        if await is_cancel(msg, number.text):
            return
        phone = number.text
        confirm = await pbot.ask(chat.id, f'`Is "{phone}" correct? (y/n):` \n\nSend: `y` (If Yes)\nSend: `n` (If No)')
    ...
    try:
        ...
        await pbot.send_message(chat.id ,f"**ERROR:** `{str(e)}`\nPress /start to Start again.")
        return
    try:
    ...
    try:
        ...
        await client.send_message("me", f"#PYROGRAM #STRING_SESSION\n\n```{session_string}``` \n\nBy [@komiXrobot](tg://openmessage?user_id=1983652989) \nA Bot By @komiXrobot")
        await client.disconnect()
        ...
        await pbot.send_message(chat.id, text, reply_markup=reply_markup)
    except Exception as e:
        await pbot.send_message(chat.id ,f"**ERROR:** `{str(e)}`")
        return
