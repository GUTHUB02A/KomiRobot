...
from ShoukoRobot import (DEV_USERS, OWNER_ID, DRAGONS, SUPPORT_CHAT, DEMONS,
...
from ShoukoRobot.modules.log_channel import gloggable
from telegram import ParseMode, TelegramError, Update
from telegram.ext import CallbackContext, CommandHandler, run_async
from telegram.utils.helpers import mention_html

ELEVATED_USERS_FILE = os.path.join(os.getcwd(),
                                   'ShoukoRobot/elevated_users.json')
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        rt += "This member is my Sensei, but this user mistake so best friend."
        data['sudos'].remove(user_id)
        DRAGONS.remove(user_id)

    if user_id in DEMONS:
        rt += "This user is my Senpai, but this user mistake so best friend."
        data['supports'].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in TIGERS:
        message.reply_text("This user is already my best friend.")
        return ""
    ...
    update.effective_message.reply_text(
        rt +
        f"\nNow {user_member.first_name} Become my Best friend!"
    )
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Requested Onichan to throw this users out of everywhere")
        DRAGONS.remove(user_id)
        data['sudos'].remove(user_id)
...
SUDO_HANDLER = CommandHandler("addsensei", addsudo)
SUPPORT_HANDLER = CommandHandler("addsenpai", addsupport)
TIGER_HANDLER = CommandHandler("addbf", addtiger)
WHITELIST_HANDLER = CommandHandler("addf", addwhitelist)
UNSUDO_HANDLER = CommandHandler("removesensei", removesudo)
UNSUPPORT_HANDLER = CommandHandler("removesenpai",
                                   removesupport)
UNTIGER_HANDLER = CommandHandler("removebf", removetiger)
UNWHITELIST_HANDLER = CommandHandler("removef",
                                     removewhitelist)

WHITELISTLIST_HANDLER = CommandHandler("friends",
                                       whitelistlist)
TIGERLIST_HANDLER = CommandHandler("besfriends", tigerlist)
SUPPORTLIST_HANDLER = CommandHandler("senpai", supportlist)
SUDOLIST_HANDLER = CommandHandler("sensei", sudolist)
DEVLIST_HANDLER = CommandHandler(["onichan", "devlist"], devlist)
