...
from telegram import MAX_MESSAGE_LENGTH, ParseMode, Update ,  __version__ as ptbver ,InlineKeyboardButton, InlineKeyboardMarkup 
from telegram.ext import CallbackContext, CommandHandler
from telegram.ext.dispatcher import run_async
from telegram.error import BadRequest
from telegram.utils.helpers import escape_markdown, mention_html

from ShoukoRobot import (DEV_USERS, OWNER_ID, DRAGONS, DEMONS, TIGERS, WOLVES,
                          INFOPIC, dispatcher , StartTime)
from ShoukoRobot.__main__ import STATS, API_KEY, USER_INFO 
from ShoukoRobot.modules.sql import SESSION
import ShoukoRobot.modules.sql.userinfo_sql as sql
from ShoukoRobot.modules.sql.users_sql import get_all_users , get_user_com_chats
from ShoukoRobot.modules.disable import DisableAbleCommandHandler
from ShoukoRobot.modules.sql.global_bans_sql import is_user_gbanned
from ShoukoRobot.modules.sql.afk_sql import is_afk, check_afk_status
from ShoukoRobot.modules.sql.users_sql import get_user_num_chats
from ShoukoRobot.modules.sql.feds_sql import get_user_fbanlist
...
from ShoukoRobot import telethn as SaitamaTelethonClient, TIGERS, DRAGONS, DEMONS
...
def info(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if user_member.status == "administrator":
            result = requests.post(
                f"https://api.telegram.org/bot{API_KEY}/getChatMember?chat_id={chat.id}&user_id={user.id}",
            )
    ...
    if INFOPIC:
        try:
            komiuser=update.effective_user.id
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/shouko_updates/8"),
                            InlineKeyboardButton(
                                "Disaster", url="https://t.me/shouko_updates/3"),
                            InlineKeyboardButton(
                                "User", url=f"tg://user?id={komiuser}")
                        ],
                    ]
                ),
                parse_mode=ParseMode.HTML,
...
def about_me(update: Update, context: CallbackContext):
    ...
    if info:
        update.effective_message.reply_text(
            f"*{user.first_name}*: {escape_markdown(info)}",
...
INFO_HANDLER = DisableAbleCommandHandler(("info", "book" , "profile"), info)
GET_USER_COMMON_CHATS = CommandHandler("getchats" , get_user_common_chats)
...
__handlers__ = [
    ...
    SET_ABOUT_HANDLER, GET_ABOUT_HANDLER, STATS_HANDLER, GET_USER_COMMON_CHATS
]
