...
from ShoukoRobot import API_KEY, dispatcher
...
def reverse(update: Update, context: CallbackContext):
    if not update.effective_message.reply_to_message:
        ...
        get_path = requests.post(
            f"https://api.telegram.org/bot{API_KEY}/getFile?file_id={photo_id}"
        ).json()
        file_path = get_path["result"]["file_path"]
        data = {
            "imageUrl": f"https://images.google.com/searchbyimage?safe=off&sbisrc=tg&image_url=https://api.telegram.org/file/bot{API_KEY}/{file_path}"
        }
        ...
        get_path = requests.post(
            f"https://api.telegram.org/bot{API_KEY}/getFile?file_id={sticker_id}"
        ).json()
        file_path = get_path["result"]["file_path"]
        data = {
            "imageUrl": f"https://images.google.com/searchbyimage?safe=off&sbisrc=tg&image_url=https://api.telegram.org/file/bot{API_KEY}/{file_path}"
